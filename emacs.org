...
#+SETUPFILE: ~/.config/vendor/org-html-themes/org/theme-readtheorg-local.setup
...
(use-package org
  :bind (:map org-mode-map
              ("C-x C-S-s" . org-save-all-org-buffers)
...
#+end_src
*** Shortcut key
Add a keybinding for =org-speed-commands=.
From https://github.com/revrari/emacs_elements_chapter_2/blob/main/org-speed-commands.org
#+begin_src emacs-lisp
...
;;(define-key org-mode-map (kbd "C-*") 'org-jump-to-heading-beginning)
#+end_src
** Org-contrib
#+begin_src emacs-lisp
...
#+end_src
** Auto insert
Insert template text automatically into new files. In this case, use =Yasnippet= to insert the template text. Example from [[ https://www.emacswiki.org/emacs/AutoInsertMode][EmacsWiki]].
#+begin_src emacs-lisp
...
#+end_src
** Golden ratio window
A way to adjust window width to always present the current with a larger size. Keep the =golden ratio= aspect of the view.
#+begin_src emacs-lisp
...
;; Detect what version of OneDrive that is used, Business or Personal.
(defconst my/onedrive-dir (let ()
                            (cond ((getenv "OneDriveCommercial"))
                                  ((getenv "OneDrive")))))
...
;; Denote file directory
(defconst my/denote-directory (expand-file-name "notes/" my/onedrive-dir))
#+end_src
* Functions
...
#+begin_src emacs-lisp
(when (file-exists-p (expand-file-name ".config/lisp/" "~"))
  (add-to-list 'load-path (expand-file-name ".config/lisp/" "~")))
...
(set-face-attribute 'default nil
                        ...
                        :weight 'normal
                        :width  'condensed)
(set-face-attribute 'fixed-pitch nil :family "CaskaydiaCove Nerd Font Mono-11")
(set-face-attribute 'fixed-pitch-serif nil :family "CaskaydiaCove Nerd Font Mono-11")
(set-face-attribute 'variable-pitch nil :family "CaskaydiaCove Nerd Font-11")
...
#+END_SRC
*** Large files
Increase large files warning to 100MB.
#+begin_src emacs-lisp
(customize-set-variable 'large-file-warning-threshold (* 100 1024 1024))
#+end_src
** Rainbows
...
(customize-set-variable 'python-indent-guess-indent-offset t)
(customize-set-variable 'python-indent-guess-indent-offset-verbose nil)
#+end_src
** Python virtual environments
Use this to enable virtual environments in Python.
#+begin_src emacs-lisp
(use-package pyvenv)
#+end_src
** Elpy
A language mode for Python.
#+begin_src emacs-lisp
(use-package elpy
  :init (advice-add 'python-mode :before 'elpy-enable)
  :hook (elpy-mode . (lambda () (add-hook 'before-save-hook
                                         'elpy-black-fix-code nil t))))
#+end_src
** Flycheck
Code checking for various languages.
#+begin_src emacs-lisp
(use-package flycheck
  :hook ((go-mode elpy-mode emacs-lisp-mode) . flycheck-mode))
#+end_src
** edit-indirect
Neded to edit source code blocks iin Markdown.
#+begin_src emacs-lisp
...
#+end_src
** Eglot
Use =Eglot= as LSP integration.
#+begin_src emacs-lisp
(use-package eglot)
#+end_src
** Go
Use the =Go= Language.
See https://cs.opensource.google/go/x/tools/+/refs/tags/gopls/v0.13.2:gopls/doc/emacs.md
#+begin_src emacs-lisp
(use-package go-mode
  :hook
...
(use-package denote
  ...
  :init (bind-key "C-c n F" (defun denote-find-directory ()
...
(defun my/org-confirm-babel-evaluate (lang body)
  "Don't confirm code execution for LANG."
...
#+end_src
*** Load org modules
Update =org-modules= with the modules I want.
#+begin_src emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  ...
  (bind-key "C-c l" 'org-store-link dired-mode-map)
...
(setq org-capture-templates
      `(("t" "To do items" entry (file+headline my/notes-file "To Do Items")
        ...
        ;; Habit
        ("h" "New habit" entry (file+headline my/notes-file "To Do Items")
...
(use-package ox-reveal
  :custom (org-reveal-root (concat "file:///" (expand-file-name "vendor/reveal.js" "~/.config/")))
  :config
  (require 'ox-reveal))
#+end_src
** Org publish
...
#+begin_src emacs-lisp
(custom-set-variables '(org-publish-use-timestamps-flag t)
                      '(org-html-validation-link nil)
                      '(org-export-with-broken-links t)
                      '(org-html-htmlize-output-type nil))
(setq my/org-publish-directory (expand-file-name "www/Org/" "~"))
(setq my/denote-publish-directory (expand-file-name "www/Denote/" "~"))
(setq org-publish-project-alist
      `(("Org" :components ("org-notes" "org-images" "org-css" "org-pdf"))
        ...
        ;; Export notes from Denote
        ("Denote" :components ("denote-notes" "denote-images" "denote-css" "denote-pdf"))
        ("denote-notes"
         :base-directory ,my/denote-directory
         :base-extension "org"
         :publishing-directory ,my/denote-publish-directory
        ...
        ("denote-css"
         :base-directory ,(expand-file-name "css/" my/denote-directory)
         :base-extension "css\\|js"
         :publishing-directory ,(expand-file-name "css/" my/denote-publish-directory)
         :recursive t
         :publishing-function org-publish-attachment)
...
#+end_src
** Chezmoi
Use chezmoi for the dotfiles. Install the package to interact with that.
#+begin_src emacs-lisp
(use-package chezmoi
